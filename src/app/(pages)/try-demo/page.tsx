...
export default function TryDemoPage() {
  ...
  <div className="min-h-screen text-white w-full">
    <main className="pt-20 sm:pt-24 px-4 pb-12 flex flex-col items-center w-full">
      <h1 className="text-2xl sm:text-3xl font-bold mb-2 text-center leading-tight">MedVision AI Analysis</h1>
      ...
      <div className="bg-white/5 p-4 sm:p-6 md:p-8 rounded-2xl border border-white/10 w-full max-w-xl lg:max-w-4xl shadow-2xl">
        <h2 className="text-base sm:text-lg font-bold mb-2">🛡️ Data & Privacy Notice</h2>
        ...
        <div className="mb-6">
          ...
          <input
            ...
            className="block w-full text-xs sm:text-sm text-gray-400
              ...
              hover:file:bg-purple-500 transition-all cursor-pointer file:cursor-pointer"
          />
        </div>
        ...
        {saveStatus.type && (
          ...
          >
            {saveStatus.type === "success" ? (
              <span className="text-lg sm:text-xl text-green-500 flex-none">✅</span>
            ) : (
              <span className="text-lg sm:text-xl text-red-500 flex-none">❌</span>
            )}
            <p className="font-medium break-words min-w-0">{saveStatus.message}</p>
          </div>
        )}
        ...
        {analyzedImage && analysisData && (
          <div className="mt-8 sm:mt-10 border-t border-white/10 pt-6 sm:pt-8 animate-in fade-in slide-in-from-bottom-4 duration-700 w-full">
            ...
            <div ref={pdfRef} className="flex flex-col lg:flex-row gap-6 p-4 sm:p-5 rounded-xl bg-[#1e1e2f] w-full">
              {/* Image Result */}
              ...
              <div className="flex-1 w-full flex flex-col justify-center">
                ...
                <div className="space-y-3 sm:space-y-4 p-4 sm:p-5 bg-white/5 rounded-xl border border-white/10 w-full">
                  ...
                  <div className="flex justify-between items-center gap-2 text-xs sm:text-sm">
                    ...
                    <span className={`font-bold text-sm sm:text-base truncate ${analysisData.detected ? "text-purple-400" : "text-green-400"}`}>
                      {analysisData.className}
                    </span>
                  </div>

                  <div className="flex flex-col items-center justify-center pt-2 w-full">
                    <span className="text-gray-400 text-xs sm:text-sm mb-2 self-start">AI Confidence:</span>
            ...
            <div className="max-w-full items-center mt-6 space-y-3 w-full">
              ...
              >
                {isDownloading ? (
                  <span className="animate-pulse text-sm sm:text-base">⏳ Generating Document...</span>
                ) : (
