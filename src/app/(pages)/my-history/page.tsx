  ...
  if (!isAuthenticated) {
      return (
          <div className="min-h-screen bg-transparent flex flex-col items-center justify-center text-white pt-10 px-4 text-center">
              ...
              <p className="text-gray-400 mb-6 text-sm sm:text-base">Please log in to view your medical history.</p>
  ...
  return (
    <div className="min-h-screen bg-transparent text-white w-full max-w-[1400px] mx-auto pt-6 sm:pt-10 px-4 sm:px-6 pb-12">
      ...
      <div className="mb-6 sm:mb-10 text-center md:text-left">
          <h1 className="text-2xl sm:text-[28px] font-bold text-gray-100 mb-1.5 sm:mb-2 leading-tight">My Medical History</h1>
          <p className="text-gray-400 text-xs sm:text-sm max-w-xl mx-auto md:mx-0">Review your past MRI scans and clinical notes from your doctor.</p>
      </div>
      
      <section>
        {isLoading && <div className="text-blue-400 animate-pulse text-base sm:text-lg text-center md:text-left py-4">Retrieving your secure records...</div>}
        ...
        {!isLoading && !error && scans.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 sm:py-20 px-4 bg-[#121726] rounded-2xl border border-[#2a3655] text-center">
              <p className="text-gray-400 text-base sm:text-lg mb-4 sm:mb-6">You have no MRI scans in your history.</p>
              ...
              >
                ...
                <div className="w-full aspect-square bg-[#0f111a] rounded-xl mb-4 flex items-center justify-center overflow-hidden border border-gray-800 shadow-inner">
                  {scan.imageData ? (
                    ...
                    <span className="text-gray-600 text-xs sm:text-sm font-medium">No Image Data</span>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center gap-2 mt-2 mb-4">
                  <div className="min-w-0">
                      <p className="text-[9px] sm:text-[10px] text-gray-500 uppercase tracking-wider font-semibold mb-0.5">AI Detection</p>
                      <p className={`text-sm sm:text-base font-bold truncate ${scan.tumorDetected ? 'text-red-400' : 'text-green-400'}`}>
                        {scan.className}
                      </p>
                  </div>
                  <div className="text-right flex-none">
                      <p className="text-[9px] sm:text-[10px] text-gray-500 uppercase tracking-wider font-semibold mb-0.5">Certainty</p>
                ...
                <div className="mt-auto pt-4 border-t border-[#2a3655]">
                  {scan.comment ? (
                    <div className="p-3 bg-blue-900/10 rounded-lg border border-blue-500/20">
                      ...
                      <p className="text-xs sm:text-sm text-gray-300 leading-relaxed text-left break-words">{scan.comment}</p>
                    </div>
                  ) : (
