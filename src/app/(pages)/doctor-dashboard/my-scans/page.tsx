...
export default function DoctorPersonalScansPage() {
  ...
  useEffect(() => {
    ...
    const fetchMyPersonalScans = async () => {
      try {
        setIsLoading(true);
        // By deliberately omitting the '&role=doctor' query parameter, 
        // the backend will fetch the scans uploaded by this user's ID, 
        // effectively showing their personal scan history.
      ...
      <div className="min-h-screen bg-transparent text-white w-full max-w-[1400px] mx-auto pt-6 sm:pt-10 px-4 sm:px-6 pb-12">
        ...
        <div className="mb-6 sm:mb-10 text-center md:text-left">
          ...
          <p className="text-gray-400 text-xs sm:text-sm max-w-xl mx-auto md:mx-0">
            Review the MRI scans you have personally uploaded to the system.
          </p>
        </div>
                  ...
                  <div className="mt-auto pt-4 border-t border-[#2a3655]">
                    {scan.comment ? (
                      <div className="p-3 bg-purple-900/10 rounded-lg border border-purple-500/20">
                        <p className="text-[10px] sm:text-[11px] text-purple-400 font-bold mb-1 uppercase tracking-wider flex items-center gap-1.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-purple-500 flex-none"></span>
                          Clinical Note
                        </p>
