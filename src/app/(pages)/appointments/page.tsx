...
function AppointmentContent() {
  ...
  return (
    ...
    <div className="min-h-screen bg-transparent text-white font-sans pb-10 pt-10 px-6 lg:px-12 max-w-4xl mx-auto flex flex-col gap-10"> 
      ...
      <main className="w-full">
        ...
        <div className="mb-10 text-center">
          <h1 className="text-[28px] font-bold text-gray-100 mb-2">
        ...
        {isLoading && <div className="text-blue-400 animate-pulse text-lg text-center">Loading database records...</div>}
        {error && <div className="text-red-500 p-4 bg-red-900/20 border border-red-500 rounded-lg text-center">{error}</div>}
        
        {!isLoading && !error && doctors.length === 0 && (
          <div className="text-gray-400 text-center">No doctors currently available for this specialty.</div>
        )}
          ...
          <div className="space-y-5">
            {doctors.map((doc) => (
              <div key={doc._id} className="bg-[#121726] p-5 rounded-2xl border border-[#2a3655] flex flex-col md:flex-row items-center gap-6 shadow-md hover:border-[#3b4b75] transition-all">
                ...
                <div className="flex flex-col gap-2.5 w-full md:w-45 flex-none mt-4 md:mt-0">
                  
                  {doc.isBooked ? (
                     <button 
                       disabled 
                       className="bg-[#1e293b] border border-[#334155] text-gray-400 w-full py-2.5 rounded-lg text-sm font-semibold cursor-not-allowed flex justify-center items-center gap-2"
                     >
                       🔒 Slot Reserved
                     </button>
                  ) : (
...
export default function AppointmentPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-transparent flex items-center justify-center text-blue-400 text-xl font-bold animate-pulse">
